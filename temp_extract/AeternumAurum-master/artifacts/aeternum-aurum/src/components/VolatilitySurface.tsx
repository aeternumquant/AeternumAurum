...
export default function VolatilitySurface() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Project 3D grid to isometric 2D
      ...
      // Draw horizontal grid lines (column direction)
      for (let r = ROWS; r >= 0; r--) {
        ...
        const alpha = 0.06 + (r / ROWS) * 0.1;
        ctx.strokeStyle = GOLD + alpha + ")";
        ctx.lineWidth = 0.7;
        ctx.stroke();
      }

      // Draw vertical grid lines (row direction)
      for (let c = 0; c <= COLS; c++) {
        ctx.beginPath();
        for (let r = ROWS; r >= 0; r--) {
          const h = getHeight(r, c, t);
        ...
        const alpha = 0.04 + (c / COLS) * 0.06;
        ctx.strokeStyle = GOLD + alpha + ")";
        ctx.lineWidth = 0.5;
        ctx.stroke();
      }

      // Draw glowing peak dots at high-volatility regions
      for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
          const h = getHeight(r, c, t);
          if (h > 0.65) {
            const p = project(c + 0.5, r + 0.5, h);
            const intensity = (h - 0.65) / 0.35;
            const grad = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, 5 + intensity * 8);
            grad.addColorStop(0, GOLD + (0.6 * intensity) + ")");
            grad.addColorStop(1, "transparent");
            ctx.beginPath();
            ctx.arc(p.x, p.y, 5 + intensity * 8, 0, Math.PI * 2);
            ctx.fillStyle = grad;
            ctx.fill();
          }
        }
