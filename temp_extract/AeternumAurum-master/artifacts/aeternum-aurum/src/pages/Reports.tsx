        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <FadeIn>
            ...
            <p className="text-muted-foreground text-sm leading-relaxed font-light">
              Relatórios mensais, trimestrais e anuais de performance. Documentos confidenciais disponíveis apenas para parceiros autorizados.
            </p>
          </FadeIn>
        </div>
      </section>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-background">
              ...
              <div className={`group flex items-start gap-5 p-5 border transition-colors ${r.locked ? "border-white/4 bg-card/40 opacity-60" : "border-white/5 bg-card hover:bg-white/[0.02] cursor-pointer"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1.5">
                    <span className={`text-[9px] tracking-widest uppercase border px-2 py-0.5 font-sans ${typeColor[r.type] ?? "text-primary/60 border-primary/20"}`}>
                      {r.type}
                    </span>
                    <span className="text-[10px] text-muted-foreground/40 font-sans">{r.month}</span>
                  </div>
                  <h3 className={`font-display text-base tracking-wide mb-1 ${r.locked ? "text-foreground/40" : "text-foreground group-hover:text-primary/90 transition-colors"}`}>
                    {r.title}
                  </h3>
                  <p className="text-muted-foreground/60 text-xs font-light leading-relaxed">{r.desc}</p>
                </div>
                {!r.locked && (
                  <span className="text-primary/40 group-hover:text-primary transition-colors text-xs shrink-0 mt-1">↓</span>
                )}
              </div>
            </FadeIn>
          ))}
        ...
        <FadeIn delay={0.5} direction="none">
          <div className="max-w-3xl mx-auto mt-10 p-6 border border-white/5 bg-card/30 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <p className="text-sm text-foreground font-display tracking-wide mb-1">Acesso completo ao arquivo</p>
              <p className="text-[11px] text-muted-foreground/60 font-light">Relatórios anteriores a 2023 disponíveis sob solicitação.</p>
            </div>
            <NavLink
              to="/acesso"
              className="shrink-0 px-6 py-2.5 border border-primary/30 text-primary text-[9px] tracking-[0.2em] uppercase font-sans hover:border-primary/60 hover:bg-primary/5 transition-all duration-200"
            >
              Solicitar Acesso
            </NavLink>
          </div>
